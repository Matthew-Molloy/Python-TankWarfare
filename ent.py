...
class Tank(Entity):
    ...
    def checkCollision(self, dtime):
        ...
        ele = self.engine.widgetMgr.overlayManager.getOverlayElement(self.oElement + " Cannon")
        if self.cannon:
           ele.setCaption(self.oElement + " Cannon: Ready")
        else:
           ele.setCaption(self.oElement + " Cannon: RELOADING")


        ele = self.engine.widgetMgr.overlayManager.getOverlayElement(self.oElement + " Missile")        
        if self.missile:
           ele.setCaption(self.oElement + " Missile: Ready")
        else:
           ele.setCaption(self.oElement + " Missile: RELOADING")
        ...
        for key, target in self.engine.entityMgr.ents.items():
            ...
            if target.uiname == 'MISSILE' and target.tankID != self.eid:
                if self.distance.valueDegrees() < target.checkValue:
                    self.collision = True
                    target.pos.y -= 10000
                    self.health -= 10
